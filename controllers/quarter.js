...
const getAllQuarters = async (req, res) => {
    try {
        const [rows] = await pool.query('select id, quarterCode, campusName, blockName, flatType, status, createdAt, updatedAt from quarters where isActive = 1 order by createdAt desc');
...
const getQuarterByCampusAndFlatType = async (req, res) => {
    try {
        ...
        const [rows] = await pool.query('select id, quarterCode, campusName, blockName, flatType, status, createdAt, updatedAt from quarters where campusName = ? and flatType = ? and status = ? and isActive = 1 order by createdAt desc', [campusName, flatType, 'available']);
        res.status(200).json({ success: true, quarters: rows });
    } catch (error) {
        console.error('Error fetching quarters:', error);
        res.status(500).json({ success: false, message: 'Internal server error' });
...
// 1.Database design
// 2. jwt middleware, token generation, and api access control middleware
// 2.add faculty api(sending email on success with Credentials)
// 3.login api
// 4.forgot password api(sending email with reset password link)
//   reset password api is pending
// 5.add quarter api
// 6. get all quarters api
// 7. get quarters based on campus name and flatType